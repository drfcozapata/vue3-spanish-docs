...
Con la API de Opciones, podemos usar la [opción `watch`](/api/options-state#watch) para disparar una función siempre que una propiedad reactiva cambie:
...
export default {
  ...
  watch: {
    // Cada vez que cambie la pregunta, se ejecutará esta función
  ...
  methods: {
    async getAnswer() {
      ...
      try {
        ...
        this.answer = '¡Error! No se pudo acceder a la API. ' + error
      } finally {
        this.loading = false
      }
    }
...
export default {
  watch: {
    // Nota: solo rutas simples. No se admiten expresiones.
...
Con la API de Composición, podemos usar la [función `watch`](/api/reactivity-core#watch) para activar un callback siempre que una parte del estado reactivo cambie:
...
El primer argumento de `watch` puede ser de diferentes tipos de "fuentes" reactivas: puede ser un ref (incluyendo refs computadas), un objeto reactivo, una [función `getter`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Functions/get#description), o un array de múltiples fuentes:
...
`watch` es superficial por defecto: la función de callback solo se activará cuando la propiedad observada haya sido asignada con un nuevo valor - no se activará en cambios de propiedades anidadas. Si quieres que la función de callback se dispare en todas las mutaciones anidadas, necesitas usar un watcher profundo:
...
## Watchers de Ejecución Inmediata {#eager-watchers}
...
Podemos forzar la ejecución inmediata de la función de callback de un watcher declarándola usando un objeto con una función `handler` y la opción `immediate: true`:
...
## Watchers de Una Vez {#once-watchers}
...
Esto se puede simplificar con [`watchEffect()`](/api/reactivity-core#watcheffect). `watchEffect()` nos permite rastrear automáticamente las dependencias reactivas de la función de callback. El watcher anterior se puede reescribir como:
...
Podemos usar la API [`onWatcherCleanup()`](/api/reactivity-core#onwatchercleanup) <sup class="vt-badge" data-text="3.5+" /> para registrar una función de limpieza que se llamará cuando el watcher se invalide y esté a punto de volver a ejecutarse:
...
Alternativamente, una función `onCleanup` también se pasa a las funciones de callback del watcher como tercer argumento<span class="composition-api">, y a la función de efecto `watchEffect` como primer argumento</span>:
...
### Watchers Post-Vaciado {#post-watchers}
...
### Watchers Síncronos {#sync-watchers}
...
También es posible crear watchers de forma imperativa usando el [método de instancia `$watch()`](/api/component-instance#watch):
...
Esto es útil cuando necesitas configurar condicionalmente un watcher, o solo observar algo en respuesta a la interacción del usuario. También te permite detener el watcher de forma anticipada.

</div>

## Deteniendo un Watcher {#stopping-a-watcher}

<div class="options-api">

Los watchers declarados usando la opción `watch` o el método de instancia `$watch()` se detienen automáticamente cuando el componente propietario se desmonta, por lo que en la mayoría de los casos no necesitas preocuparte por detener el watcher tú mismo.

En el raro caso de que necesites detener un watcher antes de que el componente propietario se desmonte, la API `$watch()` devuelve una función para ello:
...
Para detener manualmente un watcher, usa la función de manejo devuelta. Esto funciona tanto para `watch` como para `watchEffect`:
...
Ten en cuenta que debería haber muy pocos casos en los que necesites crear watchers de forma asíncrona, y la creación síncrona debe preferirse siempre que sea posible. Si necesitas esperar algunos datos asíncronos, puedes hacer que tu lógica de observación sea condicional en su lugar:
