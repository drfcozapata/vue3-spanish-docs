...
Aunque muchos ejemplos de esta guía solo necesitan un único componente, la mayoría de las aplicaciones reales están organizadas en un árbol de componentes anidados y reutilizables. Por ejemplo, el árbol de componentes de una aplicación Todo podría verse así:
...
## Montar la Aplicación {#mounting-the-app}

Una instancia de la aplicación no renderizará nada hasta que se llame a su método `.mount()`. Este espera un argumento "contenedor", que puede ser un elemento real del DOM o un selector de cadena:
...
### Template del Componente Root en el DOM {#in-dom-root-component-template}

La template para el componente root usualmente es parte del componente mismo, pero también es posible proveer la template de forma separada escribiéndola directamente dentro del contenedor de montaje:
...
Las templates del componente root en el DOM son a menudo usadas en aplicaciones que están [usando Vue sin un paso de compilación](/guide/quick-start.md#using-vue-from-cdn). También pueden ser usadas en conjunto con frameworks del lado del servidor, donde la template root podría ser generada dinámicamente por el servidor.
...
Esto hace que `TodoDeleteButton` esté disponible para su uso en cualquier parte de nuestra aplicación. Hablaremos sobre el registro de componentes y otros tipos de recursos en secciones posteriores de la guía. También puedes revisar la lista completa de las APIs de instancia de la aplicación en la [referencia de la API](/api/application).
