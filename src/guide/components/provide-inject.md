...
app.provide(/* clave */ 'message', /* valor */ '¡hola!')
```
...
export default {
  inject: {
    /* clave local */ localMessage: {
      from: /* clave de inyección */ 'message'
    }
  }
}
```
...
### Valores de Inyección por Defecto {#injection-default-values}
...
```js
export default {
  // se requiere la sintaxis del objeto cuando se
  // declaran valores por defecto para las inyecciones
  inject: {
    message: {
      from: 'message', // opcional si se usa la misma clave para la inyección
      default: 'default value'
    },
    user: {
      // usa una función factory para valores no primitivos que son costosos
      // de crear, o aquellos que deban ser únicos por instancia de componente.
      default: () => ({ name: 'John' })
    }
  }
}
...
Para hacer que las inyecciones estén reactivamente vinculadas al proveedor, necesitamos proporcionar una propiedad computada utilizando la función [computed()](/api/reactivity-core#computed):
...
export default {
  ...
  provide() {
    return {
      // proveer explícitamente una propiedad computada
...
## Trabajar con Symbol Keys {#working-with-symbol-keys}
