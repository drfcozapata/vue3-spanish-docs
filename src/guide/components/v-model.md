...
Cuando aprendimos sobre los enlaces de entrada de formulario, vimos que `v-model` tiene [modificadores incorporados](/guide/essentials/forms#modifiers): `.trim`, `.number` y `.lazy`. En algunos casos, también podrías querer que el `v-model` en tu componente de entrada personalizado admita modificadores personalizados.
